...
def _worker_mul_fallback(start: int, end: int, a_segment: list, b_segment: list) -> list:
    return [int(ai.int_value) * int(bi.int_value) for ai, bi in zip(a_segment, b_segment)]
...
def _fits_signed_64bit_max(ints_sample: Sequence[int], threshold: int = 1000) -> bool:
    limit = (1 << 63) - 1
    for v in ints_sample[:threshold]:
...
def mul_vectors_parallel_auto_shared(a_list: Sequence,
    ...
    sample_ints = [int(a_list[i].int_value) for i in range(sample_count)] + \
                  [int(b_list[i].int_value) for i in range(sample_count)]

    can_shared = False
    if use_shared_when_possible and _fits_signed_64bit_max(sample_ints):
        try:
            for i in range(n):
                ai = int(a_list[i].int_value)
                bi = int(b_list[i].int_value)
                if ai < -(1 << 63) or ai > (1 << 63) - 1 or bi < -(1 << 63) or bi > (1 << 63) - 1:
                    raise OverflowError
            can_shared = True
    ...
    if can_shared:
        ...
        try:
            with ProcessPoolExecutor(max_workers=max_workers) as exe:
                futs = {exe.submit(_worker_mul_shared, s, e, a_shm.name, b_shm.name, n): i for i, (s, e) in enumerate(idxs)}
                segs = [None] * len(idxs)
                for fut in as_completed(futs, timeout=timeout):
                    segs[futs[fut]] = fut.result()
            for seg in segs:
                results.extend(seg)
        finally:
        ...
        avg_bitlen = sum(v.bit_length() for v in sample_ints) / (len(sample_ints) or 1)
        if avg_bitlen > 4096:
        ...
        with ProcessPoolExecutor(max_workers=target_workers) as exe:
            futs = {exe.submit(_worker_mul_fallback, s, e, a_list[s:e], b_list[s:e]): i for i, (s, e) in enumerate(idxs)}
            segs = [None] * len(idxs)
            for fut in as_completed(futs, timeout=timeout):
                segs[futs[fut]] = fut.result()
        for seg in segs:
